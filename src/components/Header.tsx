...
export function Header() {
  ...
  return (
    <>
      {/* Top banner — only on non-home pages */}
      {!isHome && (
      ...
      >
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-3 px-4 sm:h-16 sm:px-6 lg:px-8">
                    ...
                    <Link
                      ...
                      className={`flex items-center gap-1 px-4 py-2 text-[13px] font-medium uppercase tracking-[0.08em] transition-colors ${
                        active
                          ...
                          : transparent ? 'text-white/75 hover:text-white' : 'text-maison-cacao/80 hover:text-maison-brun'
                      }`}
                    >
                      {item.label}
                      {item.hasDropdown && <ChevronDown className="size-3.5 opacity-50" aria-hidden />}
                    </Link>
                  </li>
                );
              })}
          ...
          <div className="flex shrink-0 items-center gap-0.5">
            <Link href="/articles" className={`flex size-10 items-center justify-center rounded-full transition-colors ${iconColor}`} aria-label="Rechercher">
            ...
            <Link href="/panier" className={`relative flex size-10 items-center justify-center rounded-full transition-colors ${iconColor}`} aria-label="Panier">
              ...
              {cartCount > 0 && (
                <span className={`absolute right-0 top-0 flex min-h-[18px] min-w-[18px] items-center justify-center rounded-full px-1 text-[10px] font-bold ${badgeBg}`}>
        ...
        <>
          ...
          >
            <div className="flex items-center justify-between border-b border-maison-brun/10 px-5 py-4">
              <p className="font-display text-lg font-bold uppercase tracking-[0.06em] text-maison-cacao">Menu</p>
              <button
                type="button"
